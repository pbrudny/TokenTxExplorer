...
function getBlockNumberAtDate(dateString) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let upperBlock = BigInt(yield web3.eth.getBlockNumber());
        while (lowerBlock <= upperBlock) {
            const middleBlock = (lowerBlock + upperBlock) / BigInt(2);
            const block = yield web3.eth.getBlock(Number(middleBlock));
        ...
        const block = yield web3.eth.getBlock(Number(lowerBlock));
...
function findCreationBlock(fromBlock) {
    return __awaiter(this, void 0, void 0, function* () {
        const code = yield web3.eth.getCode(tokenAddress);
        if (code === '0x') {
            ...
            if (logs.length > 0) {
                ...
                for (const log of logs) {
                    if (typeof log === "object" && log.transactionHash) {
                        const txReceipt = yield web3.eth.getTransactionReceipt(log.transactionHash);
...
function getEarlyLogs(creationBlock_1) {
    return __awaiter(this, arguments, void 0, function* (creationBlock, logsCount = 100) {
        const logs = yield web3.eth.getPastLogs({
...
function main(startDate) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const blockNumber = yield getBlockNumberAtDate(startDate);
            ...
            const creationBlock = yield findCreationBlock(blockNumber);
            yield getEarlyLogs(creationBlock);
        }
        catch (error) {
